...
from tag_fields.managers import FieldTagsManager, _TaggableManager
...
class TaggableManagerTestCase(BaseTaggingTestCase):
    ...
    def test_internal_type_is_manytomany(self):
        self.assertEqual(
            FieldTagsManager().get_internal_type(), "ManyToManyField"
        )
...
class DeconstructTestCase(SimpleTestCase):
    def test_deconstruct_kwargs_kept(self):
        instance = FieldTagsManager(
            through=OfficialThroughModel, to="dummy.To"
        )
        name, path, args, kwargs = instance.deconstruct()
        new_instance = FieldTagsManager(*args, **kwargs)
        self.assertEqual(
